...
import { CalendarIcon, Star, Crown, Trophy } from "lucide-react";
import { format, parseISO, isWithinInterval, startOfMonth, endOfMonth } from "date-fns";
import { tr } from "date-fns/locale";
import { cn } from "@/lib/utils";
import type { Employee, Achievement } from "@db/schema";
...
export default function PerformancePage() {
  ...
  const findChiefStats = (isYearly: boolean) => {
    const start = isYearly ? new Date(currentDate.getFullYear(), 0, 1) : startOfMonth(currentDate);
    const end = isYearly ? new Date(currentDate.getFullYear(), 11, 31) : endOfMonth(currentDate);
    let maxChiefDays = 0;
  ...
  const monthlyBest = findEmployeeOfTheMonth();
  const monthlyChief = findChiefStats(false);
  const yearlyChief = findChiefStats(true);
    ...
    <Layout employees={employees || []} isLoading={employeesLoading}>
      <div className="container mx-auto p-4 lg:p-6 max-w-7xl space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4 bg-white p-4 rounded-lg shadow-sm">
          <h1 className="text-2xl font-bold">Performans Raporları</h1>
